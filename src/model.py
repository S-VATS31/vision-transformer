class PatchEmbeddings(torch.nn.Module):
    ...
    Each square patch will then be linearly projected and have the CLS token added.
    ...
    def __init__(self, img_size: int, patch_size: int, C_in: int, d_model: int):
        ...
        self.proj = torch.nn.Linear(self.patch_dim, d_model).to(device)

        # CLS token - learnable parameter
        self.cls_token = torch.nn.Parameter(torch.randn(1, 1, d_model).to(device)) # [1, 1, d_model]
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            x (torch.Tensor): Output tensor of [B, num_patches + 1, d_model] (includes CLS token).
        """
...
class RMSNorm(torch.nn.Module):
    """
    RMSNorm layer applied during GQA/FFN block. Normalizes input tensor based on
    it's root mean square rather than using mean/variance in LayerNorm. RMSNorm
    is a much simpler alternative with a single learnable parameter.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            x (torch.Tensor): Normalized output tensor with same shape.
        """
        with torch.amp.autocast(device_type=device.type, dtype=dtype):
...
class RoPE(torch.nn.Module):
    """
    Apply 2D rotary positional embeddings to query, key vectors. Using 2D
    positional embeddings rather than 1D for the model to have positional awareness
    ...
    def __init__(self, head_dim: int, img_size: int, patch_size: int, base: float = 10000.0):
        super().__init__()
        # Ensure head_dim is divisible by 4 for 2D RoPE (x, y, 2 rotation matrices)
        if head_dim % 4 != 0:
    ...
    def compute_sine_cosine(self, grid_size: Optional[int] = None) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Compute 2D Sine and Cosine Rotation Matrices for spatial positions.

          Args:
              grid_size (int): Grid size (height and width) of the patch grid.

          Returns:
              sin_x (torch.Tensor): Sine values for x-axis of shape [1, 1, num_patches, head_dim//4].
              cos_x (torch.Tensor): Cosine values for x-axis of shape [1, 1, num_patches, head_dim//4].
              sin_y (torch.Tensor): Sine values for y-axis of shape [1, 1, num_patches, head_dim//4].
              cos_y (torch.Tensor): Cosine values for y-axis of shape [1, 1, num_patches, head_dim//4].
        """
        with torch.amp.autocast(device_type=device.type, dtype=dtype):
    ...
    def create_rotary(self, x: torch.Tensor, sin_x: torch.Tensor, cos_x: torch.Tensor, sin_y: torch.Tensor, cos_y: torch.Tensor) -> torch.Tensor:
        """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Apply 2D rotary positional embeddings to input tensor, x.
        This is the original method that expects [B, T, d_model] input.
...
class GroupedQueryAttention(torch.nn.Module):
    """
    Grouped Query Attention (GQA) reduces memory usage while maintaining attention
    expressiveness by grouping queries Instead of computing key-value (KV) pairs
    for each individual query, GQA forms groups of queries that share the same KV pairs,
    ...
    def __init__(self, d_model: int, num_heads: int, query_groups: int, rope_module: RoPE):
        ...
        self.rope_module = rope_module # Store RoPE
...
class GQABlock(torch.nn.Module):
    """
    GQA block which applies RMSNorm, GQA, and dropout without affecting the main
    attention module. A residual is also added for smoother gradient flow.
...
class FFN(torch.nn.Module):
    ...
    It enhances the models output representation by introducing non-linearity and enabling 
    more complex feature transformations.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        with torch.amp.autocast(device_type=device.type, dtype=dtype):
            x = self.dropout(self.linear2(F.gelu(self.linear1(x), approximate="tanh")))
            return x

class FFNBlock(torch.nn.Module):
    """
    FFN block which applies RMSNorm, FFN, and dropout without affecting the main
    FFN module. A residual is also added for smoother gradient flow.

    Args:
        d_model (int): Dimensionality of the model's input/output representations.
        d_ffn (int): Dimensionality of the feed-forward network.
            Typically, d_ffn = 4 * d_model.
...
class TransformerEncoder(torch.nn.Module):
    """
    Encoder block which will be stacked. Each encoder block has the attention block and 
    FFN block applied.
    ...
    Args:
        ...
        dropout (float): Regularizes the model and helps prevent dropout.
    """
    def __init__(self, d_model: int, num_heads: int, query_groups: int, rope_module: RoPE, d_ffn: int, dropout: float = 0.15):
        super().__init__()
        self.attn_block = GQABlock(d_model, num_heads, query_groups, rope_module, dropout) # Attn block
        self.ffn_block = FFNBlock(d_model, d_ffn, dropout) # FFN block
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            x (torch.Tensor): Transformed output tensor of same shape.
        """
        with torch.amp.autocast(device_type=device.type, dtype=dtype):
            x = self.attn_block(x) # Attn block
            x = self.ffn_block(x) # FFN block
            return x
...
class VisionTransformer(torch.nn.Module):
    ...
    Args:
        args (Args): Dataclass containing all model parameters.
    """
    def __init__(self, args: Args):
        ...
        # RMSNorm
        self.rms_norm = RMSNorm(args.d_model, args.rms_norm_eps)

        # Classification head
        self.classifier = torch.nn.Linear(args.d_model, args.num_classes)
